<?php
    ...
    if( isset($_POST['date_search_from']) && isset($_POST['date_search_to']) && ($_POST['date_search_from'] != '' && $_POST['date_search_to'] != '') ){
        $date_from = date('Y-m-d', strtotime($_POST['date_search_from']));
        $date_to = date('Y-m-d', strtotime($_POST['date_search_to']));
        $where = "CAST( DateSubmitted AS DATE ) BETWEEN '$date_from' AND '$date_to' ";
        $is_searched = TRUE;
    }

    //search using tracking number
    if( isset($_POST['trackingno_search']) && $_POST['trackingno_search'] != '' ){
        $trackingno_search = $_POST['trackingno_search'];
        $where = 'DocumentTrackingNo = '.$trackingno_search;
        $is_searched = TRUE;
    }
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row header_row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">View Documents</h1>
                    </div>
                </div>
                <!-- /. ROW  -->
                <div class="row">
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#document_table').on('click','.open-dialog', function(){
                ...
                //     $('.logs-content').append($('<table>').addClass('table-logs table'));
                //     $(data_arr).each(function(index, value){
                //         console.log(value);
                //         var content = data_arr[index].LogDescription;
                //         $('.table-logs').append(
                //                             $('<tr>').append($('<td>').html(data_arr[index].LogUpdated+' <i>by</i> '+data_arr[index].Logged) )
                //                         ).append(
                //                             $('<tr>').append($('<td>').append( $("<p>") ).html( content.replace("\n","<br/>") ) )
                //                             //$('<tr>').append($('<td>').append( $("<p>") ).html( content.replace(/<br\s*\/?>/mg,"</p><p>") ) )
                //                         );
                //     });
                // });
            });
